...
public class BltManager {
   ...
   *
   */
  private void initBltManager() {
    this.context = ContextProvider.get().getContext();
  ...
  private BroadcastReceiver searchDevices = new BroadcastReceiver() {
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      for (int i = 0; i < lstName.length; i++) {
        ...
        LogUtils.e("bluetooth", keyName + ">>>" + String.valueOf(b.get(keyName)));
      }
      ...
      //状态改变时
        ...
        if(device.getName().contains("URIT-15"))
        {
   ...
   * @param status
   */
  public void clickBlt( int status) {
    switch (status) {
      case BLUE_TOOTH_SEARTH://搜索蓝牙设备，在BroadcastReceiver显示结果
        startSearthBltDevice(context);
        break;
      case BLUE_TOOTH_OPEN://本机蓝牙启用
        if (getmBluetoothAdapter() != null)
          getmBluetoothAdapter().enable();//启用
        break;
      case BLUE_TOOTH_CLOSE://本机蓝牙禁用
        if (getmBluetoothAdapter() != null)
          getmBluetoothAdapter().disable();//禁用
        break;
      case BLUE_TOOTH_MY_SEARTH://本机蓝牙可以在50s内被搜索到
        Intent discoverableIntent = new Intent(BluetoothAdapter.ACTION_REQUEST_DISCOVERABLE);
        discoverableIntent.putExtra(BluetoothAdapter.EXTRA_DISCOVERABLE_DURATION, 50);
        context.startActivity(discoverableIntent);
        break;
      case BLUE_TOOTH_CLEAR://本机蓝牙关闭当前连接
        try {
  ...
  private boolean startSearthBltDevice(Context context) {
    ...
    if (getmBluetoothAdapter().isDiscovering())
      stopSearthBltDevice();
    LogUtils.i("bluetooth", "本机蓝牙地址：" + getmBluetoothAdapter().getAddress());
    //开始搜索
